...
const orderSchema = new Schema({
  ...
  status:            { type: String, enum: ['open', 'production_complete', 'closed', 'active', 'completed'], default: 'open' },
  sampleImage:       { type: String, default: '' },
  // Financial close fields (set once at close time, never modified after)
  closedAmount:    { type: Number, default: 0 },
  closedDeduction: { type: Number, default: 0 },
...
orderSchema.index({ company: 1, deletedAt: 1, status: 1 })
...
const paymentSchema = new Schema({
  company:  { type: Schema.Types.ObjectId, ref: 'Company', required: true },
  order:    { type: Schema.Types.ObjectId, ref: 'Order', default: null }, // legacy, no longer used
  transactionType: { type: String, enum: ['receipt', 'deduction', 'payment'], default: 'receipt' },
  amount:   { type: Number, required: true, min: 0 },
  mode:     { type: String, enum: ['cash','cheque','bank','upi'], required: true },
