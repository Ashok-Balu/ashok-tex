...
router.get('/', ah(async (req, res) => {
  ...
  const rows = await Order.find(f)
    .select('orderName company status archived producedMeter rejectedMeter acceptedMeter expectedMeter ratePerMeter deductionPct totalReceived startDate endDate reedPick size manuallyCompleted productionClosed financialClosed createdAt updatedAt lossMeter averageWeightPerMeter yarnShortageEnteredAmount')
...
router.delete('/:id', ah(async (req, res) => {
  ...
  // Permanent delete must remove order-linked allocations and adjust receipt totals.
  await adjustOrderFinancialsForDelete(orderId)
...
async function adjustOrderFinancialsForDelete(orderId) {
