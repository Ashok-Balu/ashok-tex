...
async function hasDuplicateFromDate(fromDate, excludeId = null) {
...
router.get('/machine-wage-master', auth, async (req, res) => {
  try {
    const rows = await db.MachineWageMaster.find({})
...
router.post('/machine-wage-master', auth, async (req, res) => {
  try {
    ...
    if (await hasDuplicateFromDate(fromDate)) {
      return res.status(400).json({ success: false, error: 'A Machine Wage configuration already exists for this From Date' });
    }

    // Inherit wages for machine counts not explicitly set, from the most recent previous config.
...
router.put('/machine-wage-master/:id', auth, async (req, res) => {
  try {
    ...
    if (req.body?.fromDate && await hasDuplicateFromDate(fromDate, row._id)) {
      return res.status(400).json({ success: false, error: 'A Machine Wage configuration already exists for this From Date' });
    }
