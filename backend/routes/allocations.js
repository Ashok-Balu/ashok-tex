...
router.get('/company/:companyId/summary', ah(async (req, res) => {
  ...
  }).select('orderName producedMeter rejectedMeter acceptedMeter ratePerMeter deductionPct totalReceived expectedMeter status manuallyCompleted').lean()
  ...
  const paidAgg = orderIds.length
    ...
    : []
  const paidByOrder = new Map(paidAgg.map(a => [String(a._id), Number(a.total || 0)]))

  const orderRows = orders.map(o => {
    const payable = computePayable(o)
    const paid = paidByOrder.get(String(o._id)) || 0
    const produced = Number(o.producedMeter || 0)
    const expected = Number(o.expectedMeter || 0)
    const progress = expected > 0 ? Math.min(100, (produced / expected) * 100) : 0
    const isCompleted = o.status === 'completed' || o.manuallyCompleted
    return {
      _id: o._id,
