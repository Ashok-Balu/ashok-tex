...
const isNumericInput = target => {
  ...
  if (target.type === 'date') return false
  return target.type === 'number' || target.inputMode === 'numeric' || target.inputMode === 'decimal'
}
...
// Numeric UX: clear zero on focus and normalize empty back to zero on blur.
document.addEventListener('focusin', event => {
  const target = event.target
  if (!isNumericInput(target)) return
  if (target.readOnly || target.disabled) return

  const normalized = String(target.value || '').replace(/,/g, '').trim()
