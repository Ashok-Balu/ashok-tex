<template>
  <div class="page-container">
        ...
        <div v-for="group in registerGroups" :key="group.name" class="register-group">
          ...
          <v-expand-transition>
            <div v-show="isRegisterExpanded(group.name)">
              <AgTable :rowData="group.orders" :columnDefs="registerCols" height="auto" :footerRows="group.footerRows" :pagination="false" :highlightRowId="highlightOrderId" />
            </div>
          </v-expand-transition>
        </div>
      </div>
    ...
    <!-- Form Dialog -->
    <v-dialog v-model="dialog" max-width="520" persistent :fullscreen="$vuetify.display.smAndDown">
...
const rowFilterItems = [
  ...
  { title: 'Open (Yet to Start)', value: 'prod-open' },
  { title: 'In Progress', value: 'in-progress' },
  { title: 'Production Complete', value: 'prod-closed' },
  { title: 'Ready to Close', value: 'fin-open' },
  { title: 'Has Rejections', value: 'loss' },
...
function scrollToHighlightedOrder() {
  ...
  setTimeout(() => {
    const el = document.querySelector(`tr[data-row-id="${orderId}"]`)
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }, 350)
  setTimeout(() => { highlightOrderId.value = '' }, 3000)
}
...
const scopedRows = computed(() => {
  ...
  switch (rowFilter.value) {
    case 'loss':
      ...
      break
    case 'prod-open':
      rows = rows.filter(order => !order.productionClosed && Number(order.acceptedMeter || 0) <= 0)
      break
    case 'in-progress':
      rows = rows.filter(order => !order.productionClosed && Number(order.acceptedMeter || 0) > 0)
      break
    case 'prod-closed':
      rows = rows.filter(order => !!order.productionClosed && !order.financialClosed)
      break
    case 'fin-open':
      rows = rows.filter(order => !!order.productionClosed && !order.financialClosed)
      break
...
watch(registerGroups, (groups) => {
  if (!searchTerm.value) return
  const names = groups.map(group => group.name)
  expandedRegisterGroups.value = [...new Set([...expandedRegisterGroups.value, ...names])]
})
    ...
    onCellClicked: async e => {
      ...
      if (action === 'delete') {
        ...
        const ok = await confirm({
          ...
          message: `You are about to permanently delete "${orderName}" and ALL its data including production history, rejections, and nool entries. This action cannot be undone.`,
