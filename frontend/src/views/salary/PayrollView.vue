<template>
  <div class="payroll-root">
    ...
    <!-- ══ Pay Salary Dialog ══ -->
    <v-dialog v-model="payDialog" max-width="420" :fullscreen="$vuetify.display.xs">
      <v-card rounded="xl">
    ...
    <v-dialog v-model="deductDialog" max-width="440" :fullscreen="$vuetify.display.xs">
      <v-card rounded="xl">
    ...
    <v-dialog v-model="historyDialog" max-width="560" scrollable :fullscreen="$vuetify.display.xs">
      <v-card rounded="xl">
...
import { downloadPayslip, generatePayslipPdf } from '@/utils/payslipPdf'
...
async function sendPayslipWhatsApp() {
  if (!payslipData.value) return
  ...
  const fileName = `Ashok_Tex_Payslip_${d.name}_${monthName(d.month)}_${d.year}.pdf`

  const canShareFiles = navigator.canShare && navigator.canShare({ files: [new File([''], 'test.pdf', { type: 'application/pdf' })] })

  if (canShareFiles) {
    try {
      const doc = generatePayslipPdf(d)
      const blob = doc.output('blob')
      const file = new File([blob], fileName, { type: 'application/pdf' })

      await navigator.share({
        title: `Ashok Tex Payslip - ${monthName(d.month)} ${d.year}`,
        text: msg.replace(/\*/g, ''),
        files: [file],
      })
      notify('Payslip shared successfully', 'success')
    } catch (err) {
      if (err.name !== 'AbortError') {
        fallbackWhatsAppShare(phoneNum, msg, d)
      }
    }
  } else {
    fallbackWhatsAppShare(phoneNum, msg, d)
...
function fallbackWhatsAppShare(phoneNum, msg, d) {
  downloadPayslip(d)
  const fullMsg = msg + `\n\n_(PDF attached separately)_`
  window.open(`https://wa.me/${phoneNum}?text=${encodeURIComponent(fullMsg)}`, '_blank')
...
@media (max-width: 1024px) {
  .summ-kpi-row { grid-template-columns: repeat(3, 1fr); }
  .summ-filter-bar__grid { grid-template-columns: 1fr 1fr 1fr; }
  .pay-overview { grid-template-columns: repeat(2, 1fr); }
  .payslip-fund-row { grid-template-columns: repeat(2, 1fr); }
}
...
@media (max-width: 768px) {
  ...
  .hero__icon-wrap { width: 44px; height: 44px; }

  /* Nav strip becomes scrollable on mobile */
  .nav-strip {
    width: 100%;
    overflow-x: auto;
    flex-wrap: nowrap;
  ...
  .nav-btn .mr-2 { margin-right: 4px !important; }
  ...
  /* Employee summary sections */
  .summ-month-hdr { flex-direction: column; align-items: flex-start; }
  .summ-month-hdr__stats { flex-wrap: wrap; gap: 6px; }
  ...
  .summ-emp-card__totals { gap: 12px; flex-wrap: wrap; }
  .summ-week-item__row { flex-direction: column; align-items: flex-start; gap: 6px; }
  .summ-week-item__period { min-width: unset; }
  .summ-week-item__stats { flex-wrap: wrap; gap: 4px; }

  /* Cards & tables */
  .glass-card__body { padding: 12px; }
  .glass-card__header { padding: 12px 14px; font-size: 13px; }
  .glass-card__header--between { flex-direction: column; align-items: flex-start; gap: 10px; }
  .salary-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; margin: 0 -12px; padding: 0 12px; }
  .emp-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; margin: 0 -12px; padding: 0 12px; }
  .payslip-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }

  /* Payslip */
  .payslip-doc { border-radius: 12px; }
  .payslip-doc__header { padding: 16px; }
  .payslip-doc__emp { padding: 12px 16px; font-size: 13px; }
  .payslip-section-title { padding: 12px 16px 6px; }
  .payslip-summary { padding: 8px 16px 12px; }
  .payslip-status-bar { margin: 0 16px 16px; }
  .payslip-fund-box { margin: 0 16px 16px; padding: 12px; }
  .payslip-fund-row { grid-template-columns: 1fr; gap: 10px; }
  .payslip-outstanding-bar { flex-direction: column; gap: 8px; align-items: stretch; padding: 12px 16px; }
  .payslip-outstanding-bar__sep { text-align: center; }
  .payslip-outstanding-bar__total { margin-left: 0; }

  /* Payments overview */
  .pay-overview { grid-template-columns: 1fr 1fr; gap: 10px; }
  .pay-overview__card { padding: 14px 16px; gap: 10px; }
  .pay-overview__val { font-size: 16px; }

  /* Payment info */
  .payment-info-box { font-size: 13px; }
  .hist-summary-row { flex-direction: column; gap: 10px; }

  /* Week row blocks */
  .week-row-block { padding: 12px 14px; }
  .week-row-block__body { flex-direction: column; align-items: stretch; gap: 10px; }
  .week-row-days { display: grid; grid-template-columns: repeat(4, 1fr); gap: 4px; }
  .week-row-summary { flex-wrap: wrap; }

  /* Dual progress bars on mobile */
  .dual-progress { min-width: 120px; }

  /* Expansion panels */
  .entries-panels { border-radius: 12px; }
}

/* ══ Responsive – Small Phone ════════════════════════════════════════════════ */
@media (max-width: 480px) {
  .payroll-root { padding: 8px; }

  .hero { padding: 16px; border-radius: 12px; }
  .hero__title { font-size: 16px; }
  .hero__icon-wrap { width: 40px; height: 40px; }

  /* Nav buttons even smaller */
  .nav-btn { padding: 8px 10px; font-size: 11px; min-height: 40px; }
  .nav-btn .v-icon { font-size: 16px !important; }

  /* Single column grids */
  .pay-overview { grid-template-columns: 1fr; }
  .payslip-quick-stats { grid-template-columns: 1fr 1fr; }
  .summ-filter-bar__grid { grid-template-columns: 1fr; }
  .summ-kpi-row { grid-template-columns: 1fr 1fr; }
  .summ-emp-card__totals { grid-template-columns: 1fr 1fr; display: grid; }
  .summ-grand-stats { gap: 12px; }

  /* Mode selection */
  .mode-selection__title { font-size: 18px; }
  .mode-card { padding: 20px 16px; }
  .mode-card__icon { width: 56px; height: 56px; }

  /* Week row days */
  .week-row-days { grid-template-columns: repeat(3, 1fr); }
  .week-row-day { min-width: 0; padding: 3px 1px; }

  /* Payslip */
  .payslip-fund-row { gap: 8px; }
  .payslip-doc__header { flex-direction: column; gap: 8px; }
  .payslip-doc__company { font-size: 16px; }
  .payslip-quick-stat__val { font-size: 18px; }
  .payslip-quick-stat { padding: 14px 8px 12px; }
  .payslip-summary__row { font-size: 13px; }

  /* Dialogs */
  .dialog-header { font-size: 14px; padding: 14px 16px; }

  /* Employee table */
  .emp-table { min-width: 560px; }
  .emp-table td { padding: 10px 12px; }
  .emp-table th { padding: 8px 12px; }

  /* Summary section */
  .summ-emp-cards { padding: 10px; gap: 10px; }
  .summ-emp-card__hdr { padding: 12px 14px; }
  .summ-week-item__row { padding: 8px 14px; }
  .summ-week-item__days { padding: 8px 14px 12px; gap: 6px; }
  .summ-day-cell { min-width: 50px; padding: 6px 6px; }
  .summ-day-cell__val { font-size: 11px; }

  /* Salary table adjustments */
  .salary-table { min-width: 1000px; }
  .salary-table--compact { min-width: 750px; }
  .wage-input { width: 48px; font-size: 12px; padding: 4px 4px; }
  .sticky-col { min-width: 120px; padding-left: 10px !important; }

  /* Payment overview cards */
  .pay-overview__card { border-radius: 12px; }
  .pay-overview__label { font-size: 10px; }
}
</style>
